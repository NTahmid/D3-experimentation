...
export default function lasso() {
  ...
  function lasso(_this) {
    ...
    let g = _this.append("g").attr("class", "lasso");

    // add the drawn path for the lasso
    let dyn_path = g.append("path").attr("class", "drawn");

    // add a closed path
    let close_path = g.append("path").attr("class", "loop_close");

    // add an origin node
    let origin_node = g.append("circle").attr("class", "origin");
